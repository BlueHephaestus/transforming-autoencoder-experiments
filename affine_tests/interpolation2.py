"""
MK2 - Now with bilinear interpolation. 
This is much better at handling lossy affine transformations, 
    and has proven to work very well with our random affine transformation matrix generation algorithm.
Of course, all these algorithms can only work for so far, considering if I scale it by 4x there is obviously going to be some holes,
    regardless of if I use bilinear or integer interpolation.
    ...
    for j in range(w):
        ...
        for n in range(h):
            for m in range(w):
                ...
                new_pix += src_img[n,m] * np.maximum(0, 1-np.abs(dst_x[n,m] - i)) * np.maximum(0, 1-np.abs(dst_y[n,m] - j))
        dst_img[i,j] = new_pix
