...
model:add(nn.Linear(28*28, 28*28))
model:add(nn.Reshape(28, 28))
...
--
--OI FUTURE SELF
--Decide if we want to keep using torch or use mother fucking keras
--If torch
--  see if we can get this system working with the built in sgd optimizer, 
--  if so
--    see if it is still efficient
--    if so
